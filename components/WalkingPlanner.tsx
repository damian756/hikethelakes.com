  ...
  return (
    <div className="space-y-8">
      <div className="bg-white border border-[#E8E3D8] rounded-2xl p-6 md:p-8">
        <h3 className="font-display text-lg font-bold text-[#0D1B2A] mb-4">Filter walking circuits</h3>
        <div className="flex flex-wrap gap-4">
          <div>
            <label className="block text-xs font-semibold text-[#2C3E50]/60 uppercase tracking-wider mb-2">Days</label>
              ...
              {([2, 3, 4] as Days[]).map((d) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    days === d ? 'bg-[#0D1B2A] text-white' : 'bg-[#F8F5EE] text-[#2C3E50] hover:bg-[#E8E3D8]'
                  }`}
                >
                  {d}
                </button>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#2C3E50]/60 uppercase tracking-wider mb-2">Budget</label>
            <div className="flex gap-2">
              {(['value', 'standard', 'premium'] as Budget[]).map((b) => (
                <button
                  ...
                  onClick={() => setBudget(b)}
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                    budget === b ? 'bg-[#0D1B2A] text-white' : 'bg-[#F8F5EE] text-[#2C3E50] hover:bg-[#E8E3D8]'
                  }`}
                >
                  {b === 'value' ? '£' : b === 'standard' ? '££' : '£££'}
                </button>
      ...
      {results.length > 0 ? (
        <div className="space-y-4">
          {results.map((it) => (
            <div key={it.id} className="bg-white rounded-2xl border border-[#E8E3D8] overflow-hidden shadow-sm">
              ...
              <div className="p-6">
                <h4 className="font-display text-xl font-bold text-[#0D1B2A]">{it.title}</h4>
                <p className="text-[#1A4A30] font-medium mt-1">{it.subtitle}</p>
                <p className="text-[#2C3E50]/70 text-sm mt-3">{it.description}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {it.fells.map((slug) => (
                    <Link
                      key={slug}
                      href={`${prefix}/fells/${slug}`}
                      className="inline-flex items-center gap-1 text-sm text-[#1A4A30] font-medium hover:text-[#B8912A]"
                    >
                      <Mountain size={12} />
        ...
        <p className="text-[#2C3E50]/60 text-sm">No circuits match. Try adjusting the filters.</p>
      )}
    </div>
  );
}
